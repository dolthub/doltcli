...
def test_get_dirty_tables(create_test_table):
    ...
    write_rows(repo, created_staged, initial, import_mode=CREATE, primary_key=['id'], commit=False)
    repo.add(created_staged)

    # created, unstaged
    created_unstaged = 'created_unstaged'
    write_rows(repo, created_unstaged, initial, import_mode=CREATE, primary_key=['id'], commit=False)
